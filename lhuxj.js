     ...
     if (!(await Envs()))
         ...
         for (let index = 0; index < lhAPPArr.length; index++) {
             ...
             if (loginBack) {
                 log('【开始签到】');
                 await signin();
                 await $.wait(randomInt(5000,8000));
...
function login(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {"Device-Model":"MI 8","User-Agent":"uhome-android/5.6.4 Android/10","Source":"App","buCode":"C40501","User-Source":"PMS","Channel-Code":"U_HOME","Nonce":`${randomString(32)}`,"OS-Version":"10","App-Version":"5.6.4","Accept":"application/json","Device-Id":`8f5fb5c4f53b8d${randomString(2)}`,"App-Platform":"Android","Content-Type":"application/json; charset=utf-8","Host":"new-uhome.longfor.com","Connection":"Keep-Alive","Accept-Encoding":"gzip"},
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                let result = JSON.parse(data);
                if (result.code == 0000 && result.data.hasOwnProperty("billList")) {
